  ...
  en: {
    ...
    rows: [
      { dim: 'Core action',
        ...
        agentic: 'Systematic planning: write an explicit spec, then direct autonomous agents' },
      { dim: 'Who works this way',
        vibe: 'Non-engineers and hobbyists',
        agentic: 'Software engineers acting as system architects' },
      { dim: 'Workflow',
        vibe: 'Idea → Build',
        agentic: 'Idea → Spec → Plan → Build → Review',
        vibeSteps: ['Idea', 'Build'],
        agenticSteps: ['Idea', 'Spec', 'Plan', 'Build', 'Review'] },
  ...
  de: {
    ...
    rows: [
      { dim: 'Kernvorgehen',
        ...
        agentic: 'Systematisch planen: explizite Spec, Agenten gezielt steuern' },
      { dim: 'Wer so arbeitet',
        vibe: 'Nicht-Entwickler und Hobbyisten',
        agentic: 'Software-Engineers in der Rolle von System-Architekten' },
      { dim: 'Workflow',
        vibe: 'Idee → Bauen',
        agentic: 'Idee → Spec → Plan → Bauen → Review',
        vibeSteps: ['Idee', 'Bauen'],
        agenticSteps: ['Idee', 'Spec', 'Plan', 'Bauen', 'Review'] },
...
/* A value is prose, except the workflow — which is a sequence, and reads as
   one. Two beads against five is the argument this slide is making; as a
...
const cell = (row, side) => {
  ...
  /* Each arrow travels with the step it points at, so a run that has to wrap
     ...
     which is what the five German steps do at 1080p. */
      ...
      <style>
        ...
        ${TAG} .steps .st {
          ...
          /* The deck's legibility floor. A step is one word; at body size the
             five-step run does not fit the column in German. */
          font-size: var(--ae-fs-caption);
          line-height: var(--ae-lh-caption);
          font-weight: 500;
          color: var(--fg-ink);
        ...
        /* The arrow belongs to the chip run, not to the body text around it —
           left to inherit, it sets at body size and pushes the five-step
           German run onto a second line. */
        ${TAG} .steps .ar {
